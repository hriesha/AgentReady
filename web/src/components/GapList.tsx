...
export default function GapList({ gaps }: GapListProps) {
  if (gaps.length === 0) {
    return <p className="text-sm text-stone-500">No gaps found for this SKU.</p>;
  }
  return (
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="font-medium text-stone-900">{gap.attribute}</span>
              <span className="flex items-center gap-1 text-xs text-stone-600">
                <span
                  className="inline-block h-2 w-2 rounded-full"
                  style={{ backgroundColor: STATUS_DOTS[gap.status] ?? "#a8a29e" }}
                />
                {STATUS_LABELS[gap.status] ?? gap.status}
              </span>
            </div>
            <span className="text-xs text-stone-500">impact {gap.impact}</span>
          </div>
